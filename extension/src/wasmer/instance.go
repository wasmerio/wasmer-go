...
type Instance struct {
	...
	Exports map[string]func(...Value) (Value, error)
}
...
func NewInstance(bytes []byte) (Instance, error) {
	...
	var empty_instance Instance = Instance { instance: nil, Exports: nil }

	if C.WASMER_OK != compile_result {
		return empty_instance, NewInstanceError("Failed to compile the module.")
	}

	var exports map[string]func(...Value) (Value, error) = make(map[string]func(...Value) (Value, error))
	...
	for nth := 0; nth < number_of_exports; nth++ {
		...
		exports[exported_function_name] = func(arguments ...Value) (Value, error) {
