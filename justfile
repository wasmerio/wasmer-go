...
test:
	...
	cd wasmer
	# Run the tests.
	GODEBUG=cgocheck=2 go test -test.v $(find test -type f \( -name "*_test.go" \! -name "example_*.go" \! -name "benchmark*.go" \) ) test/imports.go test/context.go test/svm_instance.go
